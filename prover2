...
[Service]
User=$USER
ExecStart=$(which snarkos) start --nodisplay --prover ${PROVER_PRIVATE_KEY} --node 0.0.0.0:4234
Restart=always
RestartSec=10
LimitNOFILE=10000
[Install]
WantedBy=multi-user.target
" > $HOME/aleo-prover2.service
 mv $HOME/aleo-prover2.service /etc/systemd/system
...
systemctl enable aleo-prover2
systemctl restart aleo-prover2

if [[ `service aleo-prover2 status | grep active` =~ "running" ]]; then
  echo -e "Your Aleo Prover Node \e[32minstalled and works\e[39m!"
  echo -e "You can check node status by the command \e[7msystemctl status aleo-prover2\e[0m"
  echo -e "You can check Aleo Prover Node logs by the command \e[7mjournalctl -u aleo-prover2 -f -o cat\e[0m"
